...
local function IsServerInfoEnabled()
    -- Read the replicated GlobalBool from the server
    return GetGlobalBool("hagebuddne_serverinfo_enabled", false)
end

hook.Add("HUDPaint", "Hagebuddne_HUD_ServerInfo", function()
    if not IsServerInfoEnabled() then return end
    ...
    draw.RoundedBox(8, boxX, boxY, boxW, boxH, Color(0, 0, 0, 169))
